...
class Coms extends CI_Controller
{
     ...
     */
    public function getAllDocumentsByMessage()
    {
    ...
    public function new_sendToTrash()
    {
        // If not ajax go away
    ...
    // Restore message
    public function new_restoreMessage()
    {
        // If not ajax go away
        if (!$this->input->is_ajax_request()) {
            redirect("/");
    ...
    // Sending message to trash
    public function new_deleteMessage()
    {
        // If not ajax go away
        if (!$this->input->is_ajax_request()) {
            redirect("/");
     ...
     *
     */
    public function new_sendPM($to_draft = false)
    {
        ...
        foreach ($user_ids as $key => $value) {
            ...
            if ($value == "all_patient") {
                ...
                $flags[] = $value;
                unset($user_ids[$key]);
            } elseif ($value == "all_therapists") {
                // supervisor/admin
                $flags[] = $value;
                unset($user_ids[$key]);
            } elseif ($value == "all_staff") {
                // system_developer/developer
                $flags[] = $value;
                unset($user_ids[$key]);
            } elseif ($value == "all_collaborator") {
                // supervisor/admin/system_developer/developer
                $flags[] = $value;
                unset($user_ids[$key]);
            } elseif ($value == "all_carecoordinator") {
                // supervisor/admin/system_developer/developer
                $flags[] = $value;
                unset($user_ids[$key]);
            }
    ...
    public function new_sendToDraft()
    {
        $this->new_sendPM(true);
    }
    public function new_sendToReply()
    {
     ...
     */
    public function markAsReadUnread()
    {
        // If not ajax go away
        if (!$this->input->is_ajax_request()) {
            redirect("/");
